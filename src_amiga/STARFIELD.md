...
Normal translation follows absolute ship speed, bounded by the current hull's maximum, with Cobra Mk III's speed of 22 as the fixed reference in all four views. Cobra retains its visual speed range of 1..33. Faster ships therefore have faster full-throttle starfields, and slower ships have slower ones; equal absolute speeds produce equal translation rates across hulls. Zero throttle retains the same slow drift, and the increase is linear with no special boost at full throttle.

The shared depth step is `64 + floor(min(speed, hull_speed) * 2048 / 22)` in 8.8 units. At full speed it is 1088 for Anaconda (speed 11), 2112 for Cobra Mk III (22), and 2949 for Asp MkII (31). Fractional visual speed is retained instead of rounding to whole speed units. Retro rockets use the current hull's reverse speed with the same fixed Cobra reference. This changes only starfield translation; ship speed, steering and object movement are unchanged.
