...
Current values for each subsequent build are recorded in `build/verification.json`. The initial SHA-256 snapshot of every file in `src-orig` is in `original-sha256.json`; the verification script does not write to the originals.
