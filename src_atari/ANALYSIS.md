...
Current values for each subsequent build are recorded in `build/verification.json`. The initial SHA-256 snapshot of all 307 historical files is in `original-sha256.json`. The raw files are preserved in `resources/elite_atarist_source.zip`; the verification script now verifies that archive because `src-orig` contains an independently buildable conversion.
